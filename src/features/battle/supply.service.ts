...
export class SupplyService {
    ...
    private _startEffect(client: GameClient, battle: Battle, slotId: number, durationMs: number, onEnd?: () => void): void {
        ...
        battle.broadcast(new EffectStartedPacket({ nickname: user.username, effectType: slotId, durationMs, activeAfterDeath: false, effectLevel: 0 }));
        setTimeout(() => {
    ...
    public startHealing(client: GameClient, battle: Battle, maxGivenFraction: number, effectDurationMs?: number): number {
        ...
        battle.broadcast(new EffectStartedPacket({ nickname: user.username, effectType: SUPPLY_SLOT.HEALTH, durationMs, activeAfterDeath: false, effectLevel: 0 }));
